...
ask() { # ask <tool> <json-of-tool_input>
    local out
    out=$(jq -cn --arg t "$1" --arg c "$t" --argjson i "$2" \
              '{session_id:"test",cwd:$c,tool_name:$t,tool_input:$i}' | bash "$guard")
    if [[ -z $out ]]; then echo allow
    else jq -r '.hookSpecificOutput.permissionDecision' <<<"$out"; fi
}
bash_call() { ask Bash "$(jq -cn --arg c "$1" '{command:$c}')"; }
...
stage="tmux send-keys -t hpc-x 'bash .hpc/actions/0001-say-hello.sh 2>&1 | tee -a .hpc/logs/0001.log'"
...
check "hex send denied"              "deny"  "$(bash_call 'tmux send-keys -H -t hpc-x 0d')"
check "arbitrary text not staged"    "deny"  "$(bash_call "tmux send-keys -t hpc-x 'sbatch job.slurm'")"
check "no tail may ride along"      "deny"  "$(bash_call "tmux send-keys -t hpc-x 'bash .hpc/actions/0001-say-hello.sh; sbatch job.slurm'")"
check "no head may ride along"      "deny"  "$(bash_call "tmux send-keys -t hpc-x 'rm .hpc/ON; bash .hpc/actions/0001-say-hello.sh'")"
check "stage without tee allowed"   "allow" "$(bash_call "tmux send-keys -t hpc-x 'bash .hpc/actions/0001-say-hello.sh'")"
check "new-session denied"          "deny"  "$(bash_call 'tmux new-session -d -s hpc-x sbatch')"
check "missing script denied"        "deny"  "$(bash_call "tmux send-keys -t hpc-x 'bash .hpc/actions/0099-nope.sh'")"

printf '#!/usr/bin/env bash\n# purpose: p\n# target:  local\n# effect:  e\nset -euo pipefail\n' >"$t/.hpc/actions/0002-no-undo.sh"
check "missing undo: denied"         "deny"  "$(bash_call "tmux send-keys -t hpc-x 'bash .hpc/actions/0002-no-undo.sh'")"

# --- tmux verbs that run without a keypress ---------------------------------
check "run-shell denied"             "deny"  "$(bash_call 'tmux run-shell -t hpc-x "sbatch job.slurm"')"
check "paste-buffer denied"          "deny"  "$(bash_call 'tmux paste-buffer -t hpc-x')"
check "respawn-pane denied"          "deny"  "$(bash_call 'tmux respawn-pane -k -t hpc-x "sh -c sbatch"')"
check "capture-pane allowed"         "allow" "$(bash_call 'tmux capture-pane -p -t hpc-x')"
...
check "staging an aborted action denied" "deny" "$(bash_call "tmux send-keys -t hpc-x 'bash .hpc/actions/0004-retired.sh.abort'")"
